<template>
  <v-container class="containerSetup" fluid>
              ...
              <v-expansion-panel id="filtersToggle" style="background: #ebedef">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <template #label>
                        <span :class="{ activeRadio: statusRadioGroupEnabled }"
                          >Active Only</span
                        >
                      </template>
                    </v-radio>
                  ...
                  <template #actions="{ expanded }">
                    <v-btn id="filterid" title="filter" variant="outlined">
                      <v-icon
                        color="#003366"
                        class="ml-n1"
                        :nudge-down="4"
                    ...
                    <v-row no-gutters class="mb-n1">
                      <v-col cols="12" class="pb-2 pt-2 pr-0">
                        <span
                          ...
                          :style="{
                            color: item.value.isReadByExchangeContact
                              ? 'black'
                              : '#1f7cef',
                          }"
                          >{{ getSubject(item.value.subject) }}</span
                        ><span style="color: gray">
                          - {{ getLatestComment(item.value) }}</span
                        >
                      </v-col>
                    </v-row>
                    <v-row no-gutters>
                    ...
                    <v-row no-gutters>
                      <v-col class="pb-1 pt-2">
                        <v-icon
                          class="pb-1"
                          :color="
                            getStatusColor(item.value.secureExchangeStatusCode)
                          "
                          right
                          dark
                        >
    ...
    <v-navigation-drawer
      ...
      persistent
    >
      <v-card v-if="newMessageSheet" class="information-window-v-card">
...
export default {
  ...
  computed: {
    ...
    searchEnabled() {
      return (
        ...
        (this.statusSelectFilter !== "" && this.statusSelectFilter !== null)
      );
    },
    ministryContactName() {
  ...
  methods: {
    ...
    openExchange(secureExchangeID) {
      this.$router.push({
        name: "viewExchange",
        params: { secureExchangeID: secureExchangeID },
      });
    },
    messageSent() {
    ...
    getMinistryTeamName(ministryOwnershipTeamId) {
      return this.ministryTeams.find(
        (item) => item.ministryOwnershipTeamId === ministryOwnershipTeamId
      ).teamName;
    },
    setFilterStatusAll() {
    ...
    saveMessageDate() {
      this.messageDateMoment = moment(this.messageDate)
        .format("YYYY-MM-DD")
        .toString();
    },
    ...
    getLatestComment(item) {
      let content = item.commentsList.reduce((a, b) =>
        a.createDate > b.createDate ? a : b
      ).content;
    ...
    getExchanges() {
      ...
      this.headerSearchParams.createDate =
        this.messageDateMoment === null ? null : [this.messageDateMoment];
      ...
      if (this.statusSelectFilter !== null && this.statusSelectFilter !== "") {
        this.headerSearchParams.secureExchangeStatusCode = [
          this.statusSelectFilter[0].secureExchangeStatusCode,
        ];
      }

      ApiService.apiAxios
        .get(ApiRoutes.edx.EXCHANGE_URL, {
          params: {
        ...
        .catch((error) => {
          ...
          this.setFailureAlert(
            error?.response?.data?.message
              ? error?.response?.data?.message
              : "An error occurred while trying to get a list of Secure Exchanges. Please try again later."
          );
        })
        .finally(() => {

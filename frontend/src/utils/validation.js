...
export function isPresentDateAndAfter1900(
  date,
  pattern = 'uuuu/MM/dd',
  includingCurrentDate = true
) {
  const formatter = new DateTimeFormatterBuilder()
    .appendPattern(pattern)
    .toFormatter(ResolverStyle.STRICT);
  try {
    ...
    const dateBefore1900 = LocalDate.parse('1899/12/31', datePatternWithSlash);
    const maxDate = includingCurrentDate
...
export function isNotEmptyInputParams(obj) {
  ...
  return (
    groups.false?.some((v) => !!v) ||
    groups.true?.some((v) => isNotEmptyInputParams(v))
  );
}
